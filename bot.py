...
async def sync_cmd(ctx: commands.Context):
    """【緊急用】スラッシュコマンドをDiscordに手動登録する"""
    await ctx.send("🔄 スラッシュコマンドをDiscordに登録しています...")
    try:
        synced = await bot.tree.sync()
        await ctx.send(f"✅ {len(synced)} 個のスラッシュコマンドを登録しました！\n（表示されない場合はCtrl+Rでリロード）")
    except Exception as e:
        await ctx.send(f"❌ 登録に失敗しました: {e}")
